...
source "$REPO_ROOT/scripts/common-functions.sh" 2>/dev/null && get_next_audit_number "typescript" "$REPO_ROOT/docs/audits"

# Option 2: Manual check - list existing TypeScript audits
ls "$REPO_ROOT/docs/audits/typescript-audit-*.md" 2>/dev/null || echo "No existing audits"
```
...
# Option 1: Use helper function (if available)
REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
source "$REPO_ROOT/scripts/common-functions.sh" 2>/dev/null && update_audit_index "$REPO_ROOT/docs/audits" "TypeScript" "typescript-audit-NN.md" "[Grade]" "[Summary]"
```
